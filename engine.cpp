...
/*void engine::draw_obj(object& obj)
{

    //cl_mem g_triangles = obj.g_mem;//clCreateBuffer(cl::context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(triangle)*obj.tri_num, obj.tri_list, &cl::error);
    cl_mem g_c_pos   =   clCreateBuffer(cl::context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float4), &c_pos, &cl::error);
    cl_mem g_c_rot   =   clCreateBuffer(cl::context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float4), &c_rot, &cl::error);
    cl_mem g_tri_num   = clCreateBuffer(cl::context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_uint), &obj.tri_num, &cl::error);
   // std::cout << obj.tri_num


    cl::error |= clSetKernelArg(cl::kernel, 0, sizeof(cl_mem), &obj.g_mem);
    cl::error |= clSetKernelArg(cl::kernel, 1, sizeof(cl_mem), &g_screen);
    cl::error |= clSetKernelArg(cl::kernel, 2, sizeof(cl_mem), &g_c_pos);
    cl::error |= clSetKernelArg(cl::kernel, 3, sizeof(cl_mem), &g_c_rot);
    cl::error |= clSetKernelArg(cl::kernel, 4, sizeof(cl_mem), &g_tri_num);

    size_t global_ws = obj.tri_num;

    //std::cout << obj.tri_num << std::endl;

    int global_mult=1;

    size_t local=(512*global_mult);
    ...
    //clReleaseMemObject(g_triangles);
    clReleaseMemObject(g_c_pos);
    clReleaseMemObject(g_c_rot);
...
/*void engine::draw_obj(object& obj)
{
   ...
   // std::cout << obj.tri_num
    ...
    cl::error |= clSetKernelArg(cl::kernel, 4, sizeof(cl_mem), &obj.g_tri_num);
...
/*void engine::draw_bulk_objs_test()
{

    unsigned int tri_num=obj_mem_manager::tri_num;
    ...
    cl::error |= clSetKernelArg(cl::kernel2, 0, sizeof(cl_mem), &obj_mem_manager::g_tri_mem);
    cl::error |= clSetKernelArg(cl::kernel2, 1, sizeof(cl_mem), &obj_mem_manager::g_tri_fstorage);

    cl::error |= clSetKernelArg(cl::kernel2, 2, sizeof(cl_mem), &g_c_pos);
    cl::error |= clSetKernelArg(cl::kernel2, 3, sizeof(cl_mem), &g_c_rot);
    cl::error |= clSetKernelArg(cl::kernel2, 4, sizeof(cl_mem), &obj_mem_manager::g_tri_num);



    size_t global_ws = tri_num;


    int global_mult=1;

    size_t local=(1024*global_mult);
    ...
    size_t ctrl_local=1024;


    cl::error = clEnqueueNDRangeKernel(cl::cqueue, cl::kernel2, 1, NULL, &global_ws, &ctrl_local, 0, NULL, NULL);
    clFinish(cl::cqueue); ///triangles ->screen
    ...
    cl::error |= clSetKernelArg(cl::kernel, 0, sizeof(cl_mem), &obj_mem_manager::g_tri_mem);
    cl::error |= clSetKernelArg(cl::kernel, 1, sizeof(cl_mem), &obj_mem_manager::g_tri_fstorage);
    cl::error |= clSetKernelArg(cl::kernel, 2, sizeof(cl_mem), &g_c_pos);
    cl::error |= clSetKernelArg(cl::kernel, 3, sizeof(cl_mem), &g_c_rot);
    cl::error |= clSetKernelArg(cl::kernel, 4, sizeof(cl_mem), &obj_mem_manager::g_tri_num);
    cl::error |= clSetKernelArg(cl::kernel, 5, sizeof(cl_mem), &depth_buffer);
    //cl::error |= clSetKernelArg(cl::kernel, 6, sizeof(cl_mem), &g_screen);

    size_t work_dim[2]={g_size, g_size};
    cl::error = clEnqueueNDRangeKernel(cl::cqueue, cl::kernel, 2, NULL, work_dim, 0, 0, NULL, NULL);
    clFinish(cl::cqueue); ///triangles ->screen
...
void engine::draw_bulk_objs_new()
{
    ...
    //cl_uint wb_size=(cl_uint)floorf((float)obj_mem_manager::tri_num/512.0);

    //size_t size=(obj_mem_manager::tri_num) * sizeof(cl_uint);

    //std::cout << "fff " << size << std::endl;

    /*unsigned int *t = new unsigned int[obj_mem_manager::tri_num];

    memset(t, 0, sizeof(cl_uint)*tri_num);

    cl_mem obuf=clCreateBuffer(cl::context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, size, t, &cl::error);
    if(cl::error!=0)
    {
        exit(cl::error);
    }*/
    ...
    cl::error |= clSetKernelArg(cl::kernel, 3, sizeof(cl_mem), &obj_mem_manager::g_tri_num);
    cl::error |= clSetKernelArg(cl::kernel, 4, sizeof(cl_mem), &depth_buffer);
    //cl::error |= clSetKernelArg(cl::kernel, 5, sizeof(cl_mem), &g_depth_screen);
    //cl::error |= clSetKernelArg(cl::kernel, 5, sizeof(cl_mem), &obuf);
    ///triangle wh parameter, pass array of triangle shit
    //cl::error |= clSetKernelArg(cl::kernel, 5, sizeof(cl_uint), &wb_size);



    /*unsigned int *t = new unsigned int;
    clEnqueueReadBuffer(cl::cqueue, obj_mem_manager::g_tri_num, true, 0, sizeof(int), t, 0, NULL, NULL);
    printf("asdf: %u", *t);*/
    ...
    //std::cout << obj_mem_manager::tri_num;
    ...
    //std::cout << "adjusted g_w" << global_ws;



    /*cl::error = clEnqueueNDRangeKernel(cl::cqueue, cl::kernel, 1, NULL, &global_ws, &ctrl_local, 0, NULL, NULL);
    clFinish(cl::cqueue);*/ ///triangles ->screen
    ...
    //clEnqueueReadBuffer(cl::cqueue, obuf, true, 0, sizeof(int)*tri_num, t, 0, NULL, NULL);

    //for(unsigned int i=0; i<obj_mem_manager::tri_num; i++)
    //    (t[i]==0) ? 0 : printf("asdf %u \n", i);
        //printf("!!!!!!! !!!!!!!\n\n")
        //printf("asdf: %u\n", t[i]);
